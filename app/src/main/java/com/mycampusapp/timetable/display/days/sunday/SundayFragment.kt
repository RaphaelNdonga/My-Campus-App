package com.mycampusapp.timetable.display.days.sunday
...
class SundayFragment : Fragment() {
    ...
    private val sundayArgs by navArgs<SundayFragmentArgs>()
    ...
    override fun onCreateView(
        ...
        val dayCollection = courseCollection.document(courseId).collection(sunday.name)
                        ...
                        return@OnMenuItemClickListener when (it.itemId) {
                            R.id.skip_switch -> {
                                if(timetableClass.isActive) {
                                    ...
                                    dayCollection.document(timetableClass.id).set(skippedClass)
                                    viewModel.updateData(timetableClass.id, sunday, courseId)
                                }
                                true
                            }
                            else -> true
