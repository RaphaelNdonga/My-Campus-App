package com.mycampusapp.timetable.display.days.friday
...
class FridayFragment : Fragment() {
    ...
    private val fridayArgs by navArgs<FridayFragmentArgs>()
    ...
    override fun onCreateView(
        ...
        val dayCollection = courseCollection.document(courseId).collection(friday.name)
                    ...
                    return@OnMenuItemClickListener when (it.itemId) {
                        R.id.skip_switch -> {
                            if (timetableClass.isActive) {
                                ...
                                dayCollection.document(timetableClass.id).update("active", false)
                                ...
                                dayCollection.document(timetableClass.id).update("active",true)
                                viewModel.updateData(timetableClass, friday, courseId)
                                firebaseRefresh()
                            }
                            true
                        }
