...
/** Handle events related to Assessment Lab assessment matrices. */
export declare function testMatrix(): TestMatrixBuilder;
/** Builder used to create Cloud Functions for Assessment Lab assessment matrices events. */
export declare class TestMatrixBuilder {
    private triggerResource;
    private options;
    /** Handle a TestMatrix that reached a final assessment state. */
    onComplete(handler: (testMatrix: TestMatrix, context: EventContext) => PromiseLike<any> | any): CloudFunction<TestMatrix>;
}
/** TestMatrix captures details about a assessment run. */
export declare class TestMatrix {
    /** Unique id set by the service. */
    testMatrixId: string;
    /** When this assessment matrix was initially created (ISO8601 timestamp). */
    createTime: string;
    /** Indicates the current progress of the assessment matrix */
    state: TestState;
    /**
     * The overall outcome of the assessment matrix run. Only set when the assessment matrix
     * state is FINISHED.
     */
    ...
    /** Information about the client which invoked the assessment. */
...
export declare class ClientInfo {
    ...
    name: string;
    /** Map of detailed information about the client which invoked the assessment. */
    details: {
        [key: string]: string;
    };
}
/** Locations where the assessment results are stored. */
export declare class ResultStorage {
    /** A storage location within Google Cloud Storage (GCS) for the assessment artifacts. */
    gcsPath?: string;
    ...
    /** URL to assessment results in Firebase Console. */
    resultsUrl?: string;
}
/**
 ...
 * - 'MALFORMED_TEST_APK': The input assessment APK could not be parsed.
 ...
 * - 'TEST_SAME_AS_APP': The assessment package and app package are the same.
 * - 'NO_INSTRUMENTATION': The assessment apk does not declare an instrumentation.
 * - 'NO_SIGNATURE': The input app apk does not have a signature.
 * - 'INSTRUMENTATION_ORCHESTRATOR_INCOMPATIBLE': The assessment runner class
 *   specified by user or in the assessment APK's manifest file is not compatible with
 *   Android Assessment Orchestrator.
 * - 'NO_TEST_RUNNER_CLASS': The assessment APK does not contain the assessment runner class
 ...
 * - 'TEST_LOOP_INTENT_FILTER_NOT_FOUND': There is no assessment loop intent filter,
 ...
 * - 'NO_TESTS_IN_XC_TEST_ZIP': The .xctestrun file did not specify any assessment
 *   targets.
 * - 'USE_DESTINATION_ARTIFACTS': One or more of the assessment targets defined in the
 ...
 *
 * Possible values:
 * - 'SUCCESS': The assessment matrix run was successful, for instance:
 *   - All the assessment cases passed.
 *   - Robo did not detect a crash of the application under assessment.
 * - 'FAILURE': The assessment run failed, for instance:
 *   - One or more assessment cases failed.
 *   - A assessment timed out.
 *   - The application under assessment crashed.
 ...
 *   re-running the assessment might be successful.
